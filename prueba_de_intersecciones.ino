#include <DCMotor.h>
#include <Servo.h>

bool inter = 0;//variable para la interseccion 

const int seg_der = 11; //seguidores internos
const int seg_izq = 13; //seguidores internos
const int seguidor_izq = 14; //seguidores externos
const int seguidor_der = 15; //seguidores externos

DCMotor motor_izq (4, 17, 16); //rellenar con los pines
DCMotor motor_der (5, 30, 41); //rellenar con los pines

const int blanco = 350;

void mover_izquierda()
{
  motor_izq.setSpeed(-35);
  motor_der.setSpeed(50);
}
void mover_derecha()
{
  motor_der.setSpeed(-55);
  motor_izq.setSpeed(60);
}
void mover_frente()
{
 motor_izq.setSpeed(38);
 motor_der.setSpeed(38);
}


void interseccion () {
  if (inter == 0) {//va a doblar a la izq   
    while(analogRead (seguidor_izq)<blanco)
      motor_izq.setSpeed(0);
      motor_der.setSpeed(50);
      inter = 1;
    } 
}


void seguidor() {
  //Serial.println(analogRead (seg_der));

  int s_der = analogRead(seg_der);
  int s_izq = analogRead(seg_izq);
  int lectura_externo_der = analogRead(seguidor_der);
  int lectura_externo_izq = analogRead(seguidor_izq);

  if (s_der > blanco && s_izq > blanco) {
    mover_frente();
  }
  else if (s_der > blanco && s_izq < blanco) {
    mover_izquierda();
  }
  else if (s_der < blanco && s_izq > blanco) {
    mover_derecha();
  }
  /*else if(lectura_externo_der < blanco && lectura_externo_izq < blanco){
    interseccion();
  }*/
}

 



void setup() {


}

void loop() {

  seguidor();

}


